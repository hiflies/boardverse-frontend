...
export default function FilterSection({title, children}: PropsWithChildren<FilterSectionProps>) {
    ...
    return (
        <div className="flex flex-col gap-4 border-t border-outline-variant/10 pt-sm">
            ...
            <div className={`overflow-y-hidden transition-all duration-300 ${hidden ? 'max-h-0 opacity-0' : 'max-h-96 opacity-100'}`}>
                {children}
            </div>
        </div>
    );
